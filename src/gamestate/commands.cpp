...
bool can_begin_factory_building_construction(sys::state& state, dcon::nation_id source, dcon::province_id location, dcon::factory_type_id type, bool is_upgrade, dcon::factory_type_id refit_target) {
	...
	*/
	...
	if(is_upgrade) {
		...
		// Foundry's civic urban capacity is the authoritative site limit. The
		// former Alice cities-and-towns scalar above was a second, contradictory
		// gate and allowed AI code paths to disagree with the player UI.
		return true;
	}
}
