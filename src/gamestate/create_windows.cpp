...
class group_container : public container_base {
public:
	...
	void impl_on_update(sys::state& state) noexcept override {
		...
		bool no_chance = state_colonization.begin() == state_colonization.end();
		if (no_chance) return;
	...
	void impl_render(sys::state& state, int32_t x, int32_t y) noexcept override {
		if(actually_visible) {
			container_base::impl_render(state, x, y);
		}
	}	
};
...
void create_in_game_windows(sys::state& state) {
	...
	state.world.for_each_state_definition([&](dcon::state_definition_id id) {
		auto state_ptr = std::make_unique<group_container>();
		state_ptr->state_def = id;
		...
		for(uint32_t i = 0; i < 10; i++) {
			...
			state_ptr->add_child_to_front(std::move(ptr));
		}

		state.ui_state.colonization_icons_root->add_child_to_front(std::move(state_ptr));
	});
	...
	{
		//auto ptr = ;
		//state.ui_state.unit_counter = ptr.get();
		state.ui_state.unit_counter_box = ui::make_element_by_type<ui::unit_counter_window>(state, "alice_map_unit");
	}
