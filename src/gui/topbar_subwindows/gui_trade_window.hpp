...
public:
	void on_update(sys::state& state) noexcept override {
		...
		set_text(state, text::format_money(economy::price(state, n, c)));
	}
};
...
public:
	void on_update(sys::state& state) noexcept override {
		auto commodity_id = retrieve<dcon::commodity_id>(state, parent);
		if(commodity_id)
			set_text(state, text::format_float(economy::demand_satisfaction(state, state.local_player_nation, commodity_id), 2));
	}
};
...
public:
	void on_update(sys::state& state) noexcept override {
		auto commodity_id = retrieve<dcon::commodity_id>(state, parent);
		if(commodity_id)
			set_text(state, text::format_float(economy::demand(state, state.local_player_nation, commodity_id), 1));
	}
};
...
public:
	void on_update(sys::state& state) noexcept override {
		auto commodity_id = retrieve<dcon::commodity_id>(state, parent);
		set_text(state, text::format_float(economy::market_pool(state, commodity_id), 2));
	}
};
...
public:
	void on_update(sys::state& state) noexcept override {
		auto commodity_id = retrieve<dcon::commodity_id>(state, parent);
		if(commodity_id)
			set_text(state, text::format_float(commodity_id ? economy::supply(state, state.local_player_nation, commodity_id) : 0.0f, 2));
	}
};
...
public:
	void on_update(sys::state& state) noexcept override {
		auto commodity_id = retrieve<dcon::commodity_id>(state, parent);
		if(commodity_id)
			set_text(state, text::format_float(commodity_id ? economy::estimate_factory_consumption(state, commodity_id, state.local_player_nation) : 0.0f, 2));
	}
};
...
public:
	...
	void on_update(sys::state& state) noexcept override {
		...
		case trade_sort::global_market_pool:
			std::sort(row_contents.begin(), row_contents.end(), [&](dcon::commodity_id a, dcon::commodity_id b) {
				auto av = economy::market_pool(state, a);
				auto bv = economy::market_pool(state, b);
				if(av != bv)
					return av > bv;
				else
					return a.index() < b.index();
		...
		case trade_sort::real_demand:
			std::sort(row_contents.begin(), row_contents.end(), [&](dcon::commodity_id a, dcon::commodity_id b) {
				auto av = economy::demand(state, state.local_player_nation, a);
				auto bv = economy::demand(state, state.local_player_nation, b);
				if(av != bv)
					return av > bv;
				else
					return a.index() < b.index();
...
public:
	...
	void on_update(sys::state& state) noexcept override {
		row_contents.clear();
		state.world.for_each_commodity([&](dcon::commodity_id id) {
			if(id != economy::money && economy::estimate_factory_consumption(state, id, state.local_player_nation) > 0.f) {
				row_contents.push_back(id);
			}
		});
		switch(sort) {
...
public:
	void on_update(sys::state& state) noexcept override {
		...
		auto sat = economy::demand_satisfaction(state, state.local_player_nation, com);
	...
	void update_tooltip(sys::state& state, int32_t x, int32_t y, text::columnar_layout& contents) noexcept override {
		...
		if(com != economy::money) {
			...
			text::add_line(state, contents, "alice_commodity_cprice", text::variable_type::x, text::format_money(economy::price(state, com)));
			...
			text::add_line(state, contents, "alice_commodity_eprice", text::variable_type::x, text::format_money(economy::price(state, state.local_player_nation, com)));
			text::add_line_break_to_layout(state, contents);
		}
		text::add_line(state, contents, "trade_commodity_report_1", text::variable_type::x, text::fp_one_place{ economy::demand(state, com) });
		text::add_line(state, contents, "trade_commodity_report_2", text::variable_type::x, text::fp_one_place{ economy::supply(state, com) });
		text::add_line(state, contents, "trade_commodity_report_4", text::variable_type::x, text::fp_one_place{ economy::market_pool(state, com) });
		text::add_line_break_to_layout(state, contents);
		...
		for(auto n : state.world.in_nation) {
			if(economy::supply(state, n.id, com) >= 0.05f) {
				producers.push_back(tagged_value{ economy::supply(state, n.id, com), n.id });
			}
		}
		if(producers.size() > 0) {
			...
			for(auto p : state.world.in_province) {
				if(p.get_nation_from_province_ownership()) {
					r_total += state.world.province_get_rgo_output(p.id, com);
				}
			}
			float a_total = 0.0f;
			for(auto n : state.world.in_nation) {
			...
			for(auto f : state.world.in_factory) {
				if(f.get_building_type().get_output() == com)
					f_total += state.world.factory_get_output(f.id);
			}
			...
			if(a_total > 0.f) {
				...
				text::add_line(state, contents, "w_artisan_profit", text::variable_type::x, text::fp_one_place{ 0.0f });
				text::add_line(state, contents, "w_artisan_distribution", text::variable_type::x, text::fp_one_place{ 0.0f * 100.f });
			}
			if(f_total > 0.f) {
		...
		for(auto const fat_stown_id : state.world.nation_get_state_ownership(state.local_player_nation)) {
			province::for_each_province_in_state_instance(state, fat_stown_id.get_state(), [&](dcon::province_id pid) {
				...
				if(vt == trade_flow_data::value_type::produced_by)
					if(state.world.province_get_rgo_output(pid, commodity_id) > 0.f) {
						trade_flow_data td{};
	...
	void on_update(sys::state& state) noexcept override {
		...
		for(auto n : state.world.in_nation)
			if(n.get_owned_province_count() != 0)
				distribution.emplace_back(n.id, economy::supply(state, n.id, com));
		update_chart(state);
	}
};
...
public:
	void on_update(sys::state& state) noexcept override {
		...
		for(auto n : state.world.in_nation)
			if(n.get_owned_province_count() != 0)
				distribution.emplace_back(n.id, economy::demand(state, n.id, com));
		update_chart(state);
	}
};
...
public:
	void on_update(sys::state& state) noexcept override {
			...
			for(const auto pc : state.world.nation_get_province_control(state.local_player_nation)) {
				for(const auto fl : pc.get_province().get_factory_location()) {
					if(fl.get_factory().get_building_type().get_output() == com)
						amount += state.world.factory_get_output(fl.get_factory());
				}
			}
			total += amount;
		...
		}
		{
			float amount = 0.f;
			for(const auto pc : state.world.nation_get_province_control(state.local_player_nation)) {
				amount += state.world.province_get_rgo_output(pc.get_province(), com);
			}
			total += amount;
