...
void make_mod_file() {
	...
	std::thread file_maker([path]() {
		...
		if(auto f = open_file(common, NATIVE("bookmarks.txt")); f) {
			...
			fprintf(stderr, "File common/bookmarks.txt could not be opened\n");
		}

		sys::checksum_key scenario_key;

		fprintf(stdout, (std::string("Loaded ") + std::to_string(uint32_t(bookmark_context.bookmark_dates.size())) + " bookmarks\n").c_str());
		...
		for(uint32_t date_index = 0; date_index < uint32_t(bookmark_context.bookmark_dates.size()); date_index++) {
			...
			if(date_index == 0) {
				...
				// Avoid overriding the file with autogenerated name
				while(simple_fs::peek_file(sdir, generated_scenario_name)) {
					++append;
					generated_scenario_name = base_name + NATIVE("-") + std::to_string(append) + NATIVE(".bin");
				}
				...
				fprintf(stdout, std::string("Scenario " + selected_scenario_file + " built and selected\n").c_str());
		...
		if(!err.accumulated_errors.empty() || !err.accumulated_warnings.empty()) {
			...
			fprintf(stdout, "There was a number of errors with the build. You can still play the mod, but it might be unstable. Check scenario_errors.txt for the full log.");
		}
...
int create_window() {
	...
	m_window = glfwCreateWindow(base_width, base_height, "Foundry Launcher", NULL, NULL);
	if(!m_window) {
		glfwTerminate();
		return -1;
	}
...
int main(int argc, char* argv[]) {
	...
	for(int i = 1; i < argc; i++) {
		...
		if(arg == "-outputScenario" && i + 1 < argc) {
			requestedScenarioFileName = argv[i + 1];
		} else if(arg == "-modsMask" && i + 1 < argc) {
			enabledModsMask = argv[i + 1];
	...
	if(enabledModsMask != "") {
		for(auto& mod : launcher::mod_list) {
			if(mod.name_.find(enabledModsMask) != std::string::npos) {
				...
				fprintf(stdout, (std::string("Mod " + mod.name_ + " added to selection with mods mask.").c_str()));
	...
	if(autoBuild && file_is_ready.load(std::memory_order_acquire)) {
		fprintf(stdout, "Building scenario.\n");
		make_mod_file();
		find_scenario_file();
		fprintf(stdout, "Scenario compilation finished.\n");
	}
